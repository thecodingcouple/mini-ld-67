export class Ray {
    constructor(originX, originY, angle, range, world) {
        ...
        this.world = world;
        this.range = range;
        this.points = _buildRay({x: originX, y: originY, height: 0, distance: 0});
    }
    ...
    _buildRay(rayPoint) {
        ...
        let nextStep = stepX.lengthSquared < stepY.lengthSquared 
            ? this._checkForWall(stepX, 1, 0, distance, stepX.y) 
            : this._checkForWall(stepY, 0, 1, distance, stepY.x);
    ...
    _checkForWall(step, shiftX, shiftY, distance, offset) {
        ...
        step.height = this.world.getTile(step.x - dx, step.y - dy).index;
